...
full <- lm(Air_Temp ~ vegetated + dayness*ECOSTRESS, 
           data = reduced)
summary(full)
...
```{r}
# for lst I only want the overall thing
getrow <- function(file, kind, correction = "corrected"){
  ...
  row <- data.frame(date, dt, kind, correction, mean)
}
...
lst_df <- get_time_series_df(kind = "temperature", correction = "not_corrected")
...
plot_tar_ts(df = biting_df, 
            cols = c("ag_veg", "ag_nveg"), 
            newnames = c("Vegetated agriculture", "Unvegetated agriculture"), 
            title = "", 
            ylab = "Biting Rate",
            newcols = c("lightblue", "darkblue"),
            ts_type = "day")

plot_tar_ts(df = biting_df, 
            cols = c("urb_veg", "urb_nveg"), 
            newnames = c("Vegetated urban area", "Unvegetated urban area"), 
            title = "", 
            ylab = "Biting Rate",
            newcols = c("tomato", "darkred"),
            ts_type = "day")

plot_tar_ts(df = biting_df, 
            cols = c("ag_veg", "urb_veg"), 
            newnames = c("Vegetated agriculture", "Vegetated urban area"), 
            title = "", 
            ylab = "Biting Rate",
            newcols = c("darkblue", "darkred"),
            ts_type = "day")

plot_tar_ts(df = biting_df, 
            cols = c("ag_nveg", "urb_nveg"), 
            newnames = c("Unvegetated agriculture", "Unvegetated urban area"), 
            title = "", 
            ylab = "Biting Rate",
            newcols = c("lightblue", "tomato"),
            ts_type = "day")
...
biting_df <- sunify(biting_df)
```

Create a tod (time of day) 
```{r}
# this functions adds a time of day to a dataset by first taking intervals that fall within the day, defined by starting ints[[i]][1] hours after suncats[[i]][1] and ending ints[[i]][2] hours after suncats[[i]][2], and assigns all remaining times to elsecat. 
...
make_box_plot(df = biting_df, 
            cols = c("urb_veg", "urb_nveg"), 
            newnames = c("Vegetated urban", "Unvegetated urban"), 
            title = "", 
            ylab = "Biting Rate",
            newcols = c("tomato", "darkred"))
...
make_box_plot(df = biting_df, 
            cols = c("ag_veg", "urb_veg"), 
            newnames = c("Vegetated agriculture", "Vegetated urban"), 
            title = "", 
            ylab = "Biting Rate",
            newcols = c("darkblue", "darkred"))
...
make_box_plot(df = biting_df, 
            cols = c("ag_nveg", "urb_nveg"), 
            newnames = c("Unvegetated agriculture", "Unvegetated urban"), 
            title = "", 
            ylab = "Biting Rate",
            newcols = c("lightblue", "tomato"))
```
...
t tests
```{r}
create_tplot <- function(rate_df = biting_df, list_of_types = c("mean", "ag", "nag", "veg", "nveg", "urban", "urb_veg", "urb_nveg", "ag_veg", "ag_nveg"), title = "Biting rate"){ #example: biting df one hour before and two hours after sunrise
  #what I'm doing here is creating a new df with col 1 the first input in the t-test, col2 the second input, the difference between the two, and the significance. 
  plot_df <- data.frame()
  for(i in (list_of_types)){
    for(j in (list_of_types)){
      for(t in unique(rate_df$tod)){
        df <- filter(rate_df, tod == t)
        test <- t.test(df[, i], df[,j], paired = TRUE)
      plot_df <- rbind(plot_df, data.frame(tod = t, in1 = i, in2 = j, diff = test$estimate, significance = ifelse(test$p.value < 0.05, "*", "")))
      }
    }
  }
  
  ggplot(plot_df, aes(x = in1, y = in2, fill = diff)) + 
     geom_tile() + 
...
four_panel("temperature", "corrected", limits = c(0,50))
```
